...
In order to make a prediction for the detectable population of \bhsb, we aim to simulate the intrinsic population, with their present day properties:
...
\bhsb are intrinsically rare in the Milky Way. We expect that there are around 170,000 present in the Milky Way. Given that there's around $\num{6e10} \msun$ of stellar mass in the Milky Way, that implies a flat Monte Carlo sample would require one to evolve about 300,000 stellar masses of binaries to get each individual \bhs. It is therefore important design a more targeted sampling strategy. It's worth considering the reasons for the rarity of \bhsb, because it informs how we should sample them.
...
For the \stroop sampling we assume the following priors for the initial distributions of the binaries. The primary mass, $m_1$, is drawn from a Kroupa IMF \citep{Kroupa+2001:2001MNRAS.322..231K} between $5 \msun$ and $150 \msun$. The mass ratio, $q = m_2 / m_1$, is drawn from a flat distribution between $0$ and $1$ \citep{Mazeh+1992:1992ApJ...401..265M}. The orbital period, $P$, drawn from a power-law distribution with $p(P) \propto (\log_{10}(P / \rm days))^{-0.55} \in [0.15, 5.5]$ \citep{Sana+2012:2012Sci...337..444S}. The eccentricity, $e$, is drawn from a power-law distribution with $p(e) \propto e^{-0.45} \in [0, 0.9]$ \citep{Sana+2012:2012Sci...337..444S}.

We use these priors to sample populations of BH-LMSs for each metallicity bin for which we have a \texttt{MESA} grid. We then evolve these binaries with \cosmicm. This gives us the formation properties of the binaries in a given metallicity bin
...
Specifically, the number of detectable systems, $N_{\rm det}$, can be written as
\begin{equation}
    N_{\rm det} = \frac{M_{\rm SF, MW}}{N_g} \sum_{i=1}^{N_g} \eta(Z_i) \cdot \phi_i,
\end{equation}
where $M_{\rm SF, MW} \approx 10^{11}\msun$ is the total star forming mass budget of the Milky Way \citep[see Appendix B2 of][]{Wagg+2022}\footnote{Note that this greater than the present day Milky Way stellar mass, because this accounts for the fact that stars have lost mass to stellar winds and supernovae by present day.}, $\eta(Z_i)$ is the formation efficiency of the target population (either BH-LMSs or BH-HMSs) at the metallicity of the $i$th binary, and $\phi_i$ is a heaviside function that is $1$ if the binary is detectable by \gaia and $0$ otherwise (this also accounts for systems that are not in their BH-star phase at present day). The formation efficiency is defined as the number of target binaries formed per unit star forming mass:
\begin{equation}
    \eta(Z) = \frac{1}{f_{\rm sample}} \frac{\sum_i^N w_i \cdot \epsilon_i}{\sum_i^N w_i (m_{1, i} + m_{2, i})},
\end{equation}

%For the Monte Carlo sample of BH-HMSs, we can use the outputs of \cosmic (\texttt{mass\_singles} and \texttt{mass\_binaries}) to compute $f_{\rm sample}$.

where $\epsilon_i$ is a heaviside function that is $1$ if the $i$th binary is a target binary and $0$ otherwise, $w_i$ is the adaptive importance sampling weight of the $i$th binary. We also account for the fact that we have not sampled the full initial distributions in our sampling. Therefore, we define $f_{\rm sample}$, which is the fraction the full parameter space that we sampled, as
\begin{equation}\label{eq:f_sample}
    f_{\rm sample} = \frac{M_{\rm sample}}{M_{\rm full}},
\end{equation}
where $M_{\rm sample}$ is the mean mass per binary that we sampled, and $M_{\rm full}$ is the mean mass of a full draw from our initial distributions. We fully sampled orbital period, and eccentricity distributions. However, we didn't sample single stars, we only sampled binaries with primary masses above $m_{1, \rm min}$ (where $m_{1, \rm min} = 5 \msun$), and rejected systems with $m_2 < m_{2, \rm min}$ (where $m_{2, \rm min} = 0.08 \msun$). In Appendix~\ref{app:f_sample} we show how to analytically compute this value and that $f_{\rm sample} \sim 0.31$ for our chosen parameters.

We use this method to calculate $N_{\rm det}$ for both the BH-HMS and BH-LMS populations, and then combine them to get the total number of detectable \bhss in the Milky Way. These totals also allow us to combine the two populations consistently, by scaling the BH-HMS and BH-LMS populations by their relative rates.

\section{Variations}

The plan for variations is currently all permutations of the following parameters:
\begin{itemize}
    \item Initial mass ratio distribution: flat, $p(q) \propto q^{-1}$
    \item Natal kicks: fallback-modulated, no kicks, no fallback-modulation
    \item Remnant mass prescriptions: \citet{Fryer+2012:2012ApJ...749...91F} delayed, \citet{Mandel+2020:2020MNRAS.499.3214M}
\end{itemize}

\tom{Note to self: the different mass ratio distribution variation has a different $f_{\rm sample}$, so we need to recalculate that for the normalisation.}
...
We retrieve an expression for $f_{\rm sample}$ by combining Eqs.~\eqref{eq:scenario1} and~\eqref{eq:scenario2}. The integrals can be evaluated numerically for any choice of IMF $\zeta(m)$, binary fraction $f_{\rm bin}(m)$, and mass limits.

\begin{equation}
    f_{\rm sample} = \frac{\displaystyle\int_{m_1^{\rm min}}^{m_1^{\rm max}}
      \zeta(m_1)\,f_{\rm bin}(m_1)
      \left(\frac{3}{2}m_1 - m_2^{\rm min}
            - \frac{(m_2^{\rm min})^2}{2 m_1}\right)\mathrm{d}m_1}{\displaystyle\int_{m_1^{\rm min, IMF}}^{m_1^{\rm max}}
        \zeta(m_1)\left[m_1 + f_{\rm bin}(m_1)
        \left(\frac{1}{2}m_1 - m_2^{\rm min}
                - \frac{(m_2^{\rm min})^2}{2 m_1}\right)\right]\mathrm{d}m_1}
\end{equation}
...
\begin{equation}
    f_{\rm sample, general} = \frac{\displaystyle\int_{m_1^{\rm min}}^{m_1^{\rm max}} \zeta(m_1)\,f_{\rm bin}(m_1) m_1 \int_{m_2^{\rm min}/m_1}^{1} (1 + q) p(q) \dd{q} \dd{m_1}}{\displaystyle\int_{m_1^{\rm min, IMF}}^{m_1^{\rm max}} \int_{m_2^{\rm min}/m_1}^{1} \qty[ f_{\rm bin}(m_1) \cdot m_1 (1 + q) + (1 - f_{\rm bin}(m_1)) m_1 ]  \zeta(m_1) p(q) \dd{q} \dd{m_1}}
\end{equation}
