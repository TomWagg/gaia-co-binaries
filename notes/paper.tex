...
\begin{equation}\label{eq:f_sample}
    f_{\rm sample} = \frac{\avg{M_{\rm sample}}}{\avg{M_{\rm full}}},
\end{equation}
where $\avg{M_{\rm sample}}$ is the average mass of the binaries that we sampled, and $\avg{M_{\rm full}}$ is the average mass of the binaries that we would have sampled if we had drawn from the full initial distributions. We fully sampled orbital period, and eccentricity distributions. However, we didn't sample single stars, we only sampled binaries with primary masses above $m_{1, \rm min}$ (where $m_{1, \rm min} = 5 \msun$), and rejected systems with $m_2 < m_{2, \rm min}$ (where $m_{2, \rm min} = 0.08 \msun$). In Appendix~\ref{app:f_sample} we show how to analytically compute $f_{\rm sample}$, which is $\sim \insertNumber$ for our chosen parameters.
...
\section{Calculating the fraction of the initial distributions sampled in our simulations}
\label{app:f_sample}

In this Section we derive the fraction of the initial distributions that were sampled in our \stroop simulations of \bhsb, $f_{\rm sample}$. As we note in Eq.~\ref{eq:f_sample}, this is the ratio of the average mass of the binaries that we sampled to the average mass of the binaries that we would have sampled if we had drawn from the full initial distributions.

We derive these average total masses using conditional probabilities. We assume primary masses are sampled from an initial mass function (IMF) $\zeta(m)$, in which a mass-dependent fraction $f_{\rm bin}(m_1)$ of primaries host a companion. Companions are assigned a uniform mass ratio $q \sim \mathcal{U}(0,1)$ with $m_2 = m_1 q$, and any binary with $m_2 < m_2^{\rm min}$ is rejected. We consider the two scenarios separately: (1) the sample population, and (2) the full population of singles and binaries.

\subsection{Conditional expectation of the mass ratio}

At fixed primary mass $m_1$, the rejection $m_2 = m_1 q > m_2^{\rm min}$ is equivalent to conditioning $q \sim \mathcal{U}(0,1)$ on $q > a$, where we define
\begin{equation}
  a \equiv \frac{m_2^{\rm min}}{m_1}.
\end{equation}
A uniform distribution conditioned on $q > a$ is simply $\mathcal{U}(a,1)$, whose mean is
\begin{equation}
  \mathbb{E}[q \mid m_1,\ \text{surviving}] = \frac{1+a}{2}.
  \label{eq:Eq}
\end{equation}
The probability that a binary with primary $m_1$ is deemed valid is the width of the surviving interval,
\begin{equation}
  P_{\rm valid, bin}(m_1) = 1 - a = 1 - \frac{m_2^{\rm min}}{m_1}.
  \label{eq:Psurv}
\end{equation}
...
Here we retain only binaries and impose $m_1 > m_1^{\rm min}$. Because we draw $q$ once and discard failures, the `valid' sample is skew by the validity probability~\eqref{eq:Psurv}: a primary of mass $m_1$ enters the kept sample with weight $\zeta(m_1) f_{\rm bin}(m_1) P_{\rm valid, bin}(m_1)$.

The average total mass per surviving binary is the conditional expectation
\begin{equation}
  \avg{M_{\rm sample}}
  = \frac{\displaystyle\int \zeta f_{\rm bin}\, P_{\rm valid, bin}\,
      \mathbb{E}[m_1+m_2\mid m_1]\,\mathrm{d}m_1}
         {\displaystyle\int \zeta f_{\rm bin}\, P_{\rm valid, bin}\,\mathrm{d}m_1}.
\end{equation}
Substituting~\eqref{eq:Psurv} and~\eqref{eq:binmass}, we find
\begin{equation}
  \avg{M_{\rm sample}}
  = \frac{\displaystyle\int_{m_1^{\rm min}}^{m_1^{\rm max}}
      ...
      \zeta(m_1)\,f_{\rm bin}(m_1)
      \left(1 - \frac{m_2^{\rm min}}{m_1}\right)\mathrm{d}m_1}
  \label{eq:scenario1}
\end{equation}
The three terms in the numerator are immediately interpretable: $\tfrac{3}{2}m_1$ is the mean total mass for an unconstrained uniform-$q$ binary (since $\langle q \rangle = \tfrac12$), while the remaining terms subtract the contribution of the truncated low-$q$ tail. Note that a \emph{constant} $f_{\rm bin}$ cancels between numerator and denominator; the binary fraction survives in~\eqref{eq:scenario1} only because it is mass-dependent and thereby reshapes the primary-mass distribution of the binary subpopulation. Similarly, setting $m_2^{\rm min} \to 0$ recovers the unconstrained mean $\langle M_{\rm sample} \rangle = \int \zeta f_{\rm bin}\,(3/2)m_1\,\mathrm{d}m_1 / \int \zeta f_{\rm bin}\,\mathrm{d}m_1$.

\subsection{Scenario 2: full population}
\label{app:scenario2}

Here we retain both singles and binaries and integrate down to the true IMF floor, $m_1 > m_2^{\rm min}$. Per drawn primary, the system is a single with probability $1 - f_{\rm bin}(m_1)$ (mass $m_1$, always kept) or a binary with probability $f_{\rm bin}(m_1)$ (mass $m_1 + m_2$, kept with probability $P_{\rm surv}$). Combining both channels, the expected retained star forming mass contribution per drawn primary is
\begin{align}
    M_{\rm SF} &= (1 - f_{\rm bin}) m_1
...
The corresponding retention probability per drawn primary combines the two
channels' valid probabilities,
\begin{equation}
    P_{\rm valid, full} = (1 - f_{\rm bin})\cdot 1 + f_{\rm bin} P_{\rm valid, bin}
  = 1 - f_{\rm bin}(m_1)\,\frac{m_2^{\rm min}}{m_1},
  \label{eq:den2}
\end{equation}
such a draw is rejected only if it is a binary \emph{and} fails the cut. Singles therefore drop out of the denominator correction entirely. Dividing the integral of~\eqref{eq:num2} by that of~\eqref{eq:den2} gives the mean total mass per surviving system,
\begin{equation}
  \langle M_{\rm full} \rangle
  = \frac{\displaystyle\int_{m_2^{\rm min}}^{m_1^{\rm max}}
      \zeta(m_1)\left[m_1 + f_{\rm bin}(m_1)
      \left(\frac{1}{2}m_1 - m_2^{\rm min}
            - \frac{(m_2^{\rm min})^2}{2 m_1}\right)\right]\mathrm{d}m_1}
         {\displaystyle\int_{m_2^{\rm min}}^{m_1^{\rm max}}
      \zeta(m_1)\left[1 - f_{\rm bin}(m_1)\,\frac{m_2^{\rm min}}{m_1}\right]
      \mathrm{d}m_1}
  \label{eq:scenario2}
\end{equation}
